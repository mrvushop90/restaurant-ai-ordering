...
async function getDeepgramTranscript(context, recordingUrl) {
  ...
  const accountSid = String(context.ACCOUNT_SID || "").trim();
  const authToken = String(context.AUTH_TOKEN || "").trim();

  if (!apiKey) {
    throw new Error("Missing DEEPGRAM_API_KEY");
  }

  // In Twilio Functions, ACCOUNT_SID and AUTH_TOKEN are available when
  // "Add my Twilio Credentials" is enabled in Environment Variables.
  ...
  const authenticatedUrl = audioUrl.replace(
    "https://",
    `https://${accountSid}:${authToken}@`
  );

  const deepgramResponse = await fetch("https://api.deepgram.com/v1/listen?model=nova-3&language=multi&smart_format=true&detect_language=true&keyterm=pho&keyterm=com+tam&keyterm=bun&keyterm=banh&keyterm=nuoc+cham", {
    method: "POST",
    headers: {
  ...
  if (!deepgramResponse.ok) {
    ...
    throw new Error(`Deepgram request failed: ${deepgramResponse.status} ${errorText}`);
  }

  const data = await deepgramResponse.json();
  const duration = data &&
    data.metadata &&
    Number.isFinite(Number(data.metadata.duration))
    ? Number(data.metadata.duration)
    : 0;
...
exports.handler = async function(context, event, callback) {
  ...
  try {
    ...
    try {
      ...
      let deepgramResult = await getDeepgramTranscript(context, recordingUrl);
      speechResult = deepgramResult.transcript;

      if (!speechResult || speechResult.trim().length < 2 || Number(deepgramResult.duration) === 0) {
        console.log("DEEPGRAM RETRY: empty transcript or zero duration");
        await new Promise((resolve) => setTimeout(resolve, 1000));
        deepgramResult = await getDeepgramTranscript(context, recordingUrl);
        speechResult = deepgramResult.transcript;
      }
