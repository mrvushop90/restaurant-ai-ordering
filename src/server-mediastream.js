...
async function getOpenAIResponse(session) {
...
function buildStreamUrl(req) {
  return `${getPublicBaseUrl(req).replace(/^http/, "ws")}/mediastream`;
}
...
async function sendAudioToTwilio(connection, text, finalize) {
  ...
  sendWsJson(connection.ws, {
    ...
    streamSid: connection.streamSid,
    mark: {
      name: finalize ? "final-response" : "assistant-response"
    }
  });
  ...
  if (finalize) {
    setTimeout(() => {
      try {
        ...
        console.log("WS CLOSE ERROR:", error.message);
      }
      clearSession(connection.callSid);
    }, 2000);
  }
}

async function processTranscript(connection, transcript) {
  const cleanedTranscript = String(transcript || "").trim();
  if (!cleanedTranscript || cleanedTranscript.length < 2 || connection.processing) {
    return;
  }
  ...
  try {
    ...
    appendHistory(session, "user", cleanedTranscript);

    const openAIResult = await getOpenAIResponse(session);
    ...
    if (parsed.orderFinalized) {
      reply = buildFinalConfirmation(session);
      finalize = true;
    }

    appendHistory(session, "assistant", reply);
    await sendAudioToTwilio(connection, reply, finalize);
  } catch (error) {
    console.log("TRANSCRIPT PROCESS ERROR:", error.message);
    try {
      await sendAudioToTwilio(connection, "Sorry bro, give me one quick second.", false);
    } catch (ttsError) {
...
app.get("/", (req, res) => {
  res.type("text/plain").send("Restaurant AI Media Streams server is running");
});
...
wss.on("connection", (ws) => {
  const connection = createMediaConnection(ws);
  ...
  ws.on("message", async (message) => {
    try {
      ...
      if (data.event === "start") {
        connection.streamSid = data.start && data.start.streamSid ? data.start.streamSid : "";
        connection.callSid = data.start && data.start.callSid ? data.start.callSid : connection.streamSid;
      ...
      if (data.event === "stop") {
        console.log("MEDIA STREAM STOP:", connection.callSid);
        try {
          connection.deepgramConnection.finish();
        } catch (error) {
          console.log("DEEPGRAM FINISH ERROR:", error.message);
        }
        clearSession(connection.callSid);
      }
    } catch (error) {
  ...
  ws.on("close", () => {
    try {
      ...
      console.log("DEEPGRAM CLOSE ERROR:", error.message);
    }

    if (connection.callSid) {
      mediaSessions.delete(connection.callSid);
    }
  });
});

server.listen(PORT, () => {
  console.log(`Restaurant AI Media Streams server listening on port ${PORT}`);
});
