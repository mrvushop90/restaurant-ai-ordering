...
async function getDeepgramTranscript(context, recordingUrl) {
  ...
  if (!recordingResponse.ok) {
    ...
    throw new Error(`Twilio recording download failed: ${recordingResponse.status} ${errorText}`);
  }

  const audioArrayBuffer = await recordingResponse.arrayBuffer();
  const audioBuffer = Buffer.from(audioArrayBuffer);

  const deepgramResponse = await fetch("https://api.deepgram.com/v1/listen?model=nova-3&language=multi&smart_format=true", {
    method: "POST",
    headers: {
