...
const DEFAULT_MEDIASTREAM_URL = "wss://simple-ai-receptionist-v1-1792.twil.io/mediastream";
...
function buildMediaStreamUrl(context) {
  return String(context.MEDIASTREAM_URL || DEFAULT_MEDIASTREAM_URL).trim() || DEFAULT_MEDIASTREAM_URL;
}

// Twilio Functions can return the TwiML that starts a Media Stream, but the
// /mediastream WebSocket bridge itself must run on a WebSocket-capable service.
function createDeepgramStreamBridge(context) {
...
exports.handler = async function(context, event, callback) {
  ...
  const transcriptInput = String(event.Transcript || event.SpeechResult || "").trim();
  const session = getSession(callSid);
  const BASE_URL = String(context.BASE_URL || "").trim();
  const mediaStreamUrl = buildMediaStreamUrl(context);
  const spokenTexts = [];
  ...
  console.log("CALL RECEIVED:", callSid, "TRANSCRIPT:", transcriptInput);
  ...
  try {
    ...
    if (transcriptInput.trim().length < 2) {
    ...
    session.greeted = true;
    appendHistory(session, "user", transcriptInput);
